...
function msghandler(Window: HWnd; AMessage:UInt; WParam : WParam; LParam: LParam): Longint;
stdcall;
...
function MessageHandleThread(p : pointer) : DWord;
StdCall;
...
begin
  ...
  XSetWMProtocols(TheDisplay, TheDrawable, @WM_DELETE_WINDOW, 1);
...
begin
  ...
  while XPending(TheDisplay) > 0 do
    begin
...
begin
    {$IFDEF WIN32}
  Result := ogl_ReadKeyWin32;{$ENDIF}
    {$IFDEF UNIX}
  Result := ogl_ReadKeyX;{$ENDIF}
end;
...
begin
    {$IFDEF WIN32}
  Result := ogl_KeyPressedWin32;{$ENDIF}
    {$IFDEF UNIX}
  Result := ogl_KeyPressedX;{$ENDIF}
end;
...
begin
  if graphInited then
    ...
    {$IFDEF UNIX}
  ogl_SwapBuffersX;{$ENDIF}
end;

procedure GetScreenResolution(var Width, Height : Word);
begin
    {$IFDEF WIN32}
  ogl_GetScreenResolutionWin32(Width, Height);{$ENDIF}
    {$IFDEF UNIX}
  ogl_GetScreenResolutionX(Width, Height);{$ENDIF}
end;
...
begin
  ...
  if graphInited then
    begin
        {$IFDEF WIN32}
      ogl_CloseGraphWin32;{$ENDIF}
        {$IFDEF UNIX }
      ogl_CloseGraphX;{$ENDIF}
      setLength(pal,0);
      graphInited := false;
    end;
end;
